...
Public Class utilidades
    ...
    Public Function recuperaEmpresa() As DataSet
        ...
        cad = " select nom_comercial,c.nom_emp,	c.cod_emp,left(c.dir_emp,INSTR(c.dir_emp,'/')-1) as dir_emp,RIGHT(c.dir_emp,LENGTH(c.dir_emp)-INSTR(c.dir_emp,'/')) as dis_emp, left(dir_sucursal,INSTR(dir_sucursal,'-')-1) as dir_sucursal,RIGHT(dir_sucursal,LENGTH(dir_sucursal)-INSTR(dir_sucursal,'-')) as dis_sucursal ,pais_emp,obs,obs1" _
            & " from configuracion c inner join empresa e on c.cod_emp=e.cod_emp where c.activo"
